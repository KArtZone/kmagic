...
import pro.artkart.kmagic.tree.Tree.Color.B
import pro.artkart.kmagic.tree.Tree.Color.BB
import pro.artkart.kmagic.tree.Tree.Color.NB
import pro.artkart.kmagic.tree.Tree.Color.R
...
sealed class Tree<out A : Comparable<@UnsafeVariance A>> {
    ...
    private val g: (ImmutableList<A>) -> (ImmutableList<A>) -> ImmutableList<A> = { a -> { b -> a.concat(b) } }
    private val f: (ImmutableList<A>) -> (A) -> ImmutableList<A> = { l -> { a -> l.cons(a) } }

    abstract fun contains(element: @UnsafeVariance A): Boolean
    abstract fun max(): A
    abstract fun min(): A
    protected abstract fun removeMax(): Tree<A>
    abstract fun pathLengths(currentDepth: Int, depths: ImmutableList<Int>): ImmutableList<Int>
    protected abstract fun pathColors(
        currentColorImmutableList: ImmutableList<Color>,
        paths: ImmutableList<ImmutableList<Color>>
    ): ImmutableList<ImmutableList<Color>>

    fun pathLengths(): ImmutableList<Int> = pathLengths(0, ImmutableList())
    protected fun pathColors(): ImmutableList<ImmutableList<Color>> = pathColors(ImmutableList(), ImmutableList())

    abstract operator fun get(element: @UnsafeVariance A): Resolution<A>
    protected abstract fun getT(element: @UnsafeVariance A): Resolution<T<@UnsafeVariance A>>
    ...
    fun toImmutableList(): ImmutableList<@UnsafeVariance A> = foldLeft(ImmutableList(), f, g)
    ...
    private abstract class Empty<A : Comparable<A>> : Tree<A>() {
        ...
        override fun pathLengths(currentDepth: Int, depths: ImmutableList<Int>): ImmutableList<Int> = ImmutableList()

        override fun get(element: A): Resolution<A> = Resolution()

        override fun getT(element: A): Resolution<T<A>> = Resolution()
        ...
        override fun pathColors(
            currentColorImmutableList: ImmutableList<Color>,
            paths: ImmutableList<ImmutableList<Color>>
        ): ImmutableList<ImmutableList<Color>> =
            paths.cons(currentColorImmutableList)
    }
    ...
    protected class T<A : Comparable<A>> internal constructor(
        ...
        private fun balance(color: Color, left: Tree<A>, value: A, right: Tree<A>): Tree<A> = when {
            ...
            color == B && left.isTR && left.right.isTR ->
                T(
                    R, T(B, left.left, left.value, left.right.left), left.right.value,
            ...
            color == B && right.isTR && right.left.isTR ->
                T(
                    R, T(B, left, value, right.left.left), right.left.value,
            ...
            color == BB && left.isTR && left.right.isTR ->
                T(
                    B, T(B, left.left, left.value, left.right.left), left.right.value,
                    T(B, left.right.right, value, right)
                )
            ...
            color == BB && right.isTR && right.left.isTR ->
                T(
                    B, T(B, left, value, right.left.left), right.left.value,
                    T(B, right.left.right, right.value, right.right)
                )
            ...
            color == BB && left.isTNB && left.left.isTB && left.right.isTB ->
                T(
                    B, balance(B, left.left.redden(), left.value, left.right.left), left.right.value,
                    T(B, left.right.right, value, right)
                )
        ...
        override fun pathLengths(currentDepth: Int, depths: ImmutableList<Int>): ImmutableList<Int> = when {
            right.isEmpty && left.isEmpty -> depths.cons(currentDepth)
            else -> left.pathLengths(currentDepth + 1, depths).concat(
        ...
        override fun pathColors(
            ...
            when {
                right.isEmpty && left.isEmpty -> paths.cons(currentColorImmutableList.cons(color))
                else -> left.pathColors(currentColorImmutableList.cons(color), paths).concat(
                    right.pathColors(currentColorImmutableList.cons(color), paths)
                )
            }

        override fun get(element: A): Resolution<A> = when {
            element < this.value -> left[element]
            element > this.value -> right[element]
            else -> Resolution(this.value)
        }

        override fun getT(element: A): Resolution<T<A>> = when {
            element < this.value -> left.getT(element)
            element > this.value -> right.getT(element)
            else -> Resolution(this)
        }
    ...
    companion object {
        ...
        fun <A : Comparable<A>> toString(ree: Tree<A>): String {
            ...
            val Resolution = makeTable(table, ree, hPosition, vPosition)
            val sb = StringBuilder()
            for (l in Resolution.size downTo 1) {
                for (c in 0 until Resolution[0].size) {
                    sb.append(makeCell(Resolution[l - 1][c]))
                }
                sb.append("\n")
            }
            return sb.toString()
        ...
        private fun <A : Comparable<A>> makeTable(
